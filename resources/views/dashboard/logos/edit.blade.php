        ...
        $(document).ready(function () {
            $('#mainphoto').dropzone({
                ...
                init: function () {
                        ...
                        @if(!empty($logo->image))
                    var mock = { name: '{{ __('admin.logo') }}', size: 2};
                    this.emit('addedfile', mock);
                    this.emit('thumbnail', mock, '{{ $logo->logo_image }}');
        ...
        <div class="container-fluid page__heading-container">
            <div class="page__heading d-flex align-items-center">
                <div class="flex">
                    ...
                    <h1 class="m-0"> {{ trans('admin.logo') }} </h1>
                </div>
            </div>
        </div>
            ...
            <div class="card card-form__body card-body">
                <form method="post" action="{{ route('logos.update', $logo->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <input class="image_name" type="hidden" name="image" value="{{ $logo->image }}">
                    </div>
                    <div class="form-group">
